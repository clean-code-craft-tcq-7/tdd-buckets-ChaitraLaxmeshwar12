...
public class BucketRangeTest
{
	...
	public void testRangeCreater() {
		...
		//sample test
		rangeandcount= BucketRange.getRangeandReadings(new int[]{4,5});
		...
		// same numbers
		rangeandcount = BucketRange.getRangeandReadings(new int[]{5,5,5,5});
		Entry = rangeandcount.entrySet().iterator().next();
		assertEquals("5",Entry.getKey());
		assertEquals(new Integer(4),Entry.getValue());
		
		// unsorted array
		rangeandcount = BucketRange.getRangeandReadings(new int[]{2,6,4,5,3});
		Entry = rangeandcount.entrySet().iterator().next();
